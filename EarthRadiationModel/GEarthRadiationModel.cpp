...
namespace gfc
{
    ...
    GEarthRadiationModel::GEarthRadiationModel()
    {
        ...
        m_ceresDataFile = m_dataDirectory + "CERES_SYN1deg-Month_Terra-Aqua-MODIS_Ed4A_Subset_200003-201708.nc";
    ...
    void GEarthRadiationModel::setVariableName(gfc::GString varName)
    {
        m_varableName = varName;
    }
    
    
    //dump original monthly averaged ceres flux data
    void GEarthRadiationModel::dumpMonToaData(GString dir)
    {
        int test = 0 ; // waiting for setting
        
        for( int k =0 ; k< m_monToaData[test].size() ; k++ )
        {
            char tmpstr[100]={0};
            
            sprintf(tmpstr, "%s%02d/grid.data",dir.c_str(), k+1);
            GString dataFileName(tmpstr);
            
            FILE* meanFile = fopen(dataFileName.c_str(),"w+");
            ...
            for( int i = 0 ; i< 360; i++  ) //360
            {
                for( int j = 0 ; j< 180; j++ ) //180
                {
                    ...
                    fprintf(meanFile, "%8.3f ",m_monToaData[test][k].data[j][i]);
                }
                fprintf(meanFile, "\n");
            }
            fclose(meanFile);
    ...
    void GEarthRadiationModel::dumpTriGridFlux( int imonth ,GString dir)
    {
        
        for( int i = 0; i < m_level+1 ; i++ )
        {
            char filepathstr[100]={0};
            sprintf(filepathstr, "%s%02d/level%1d.txt",dir.c_str(),imonth+1,i);
            GString faceFilePath = filepathstr;
            ...
            for( int j = 0 ; j < m_TriGrid[i].size(); j++ )
            {
                ...
                fprintf(faceFile, "%9s %9s %9s %9s %9s %9s %12.4f %12.4f %12.4f %12.4f %12.4f %12.4f %12.4f %12.4f %12.4f %20.10f %20.12f %20.12f\n",
                        ...
                        area,
                        m_TriGrid[i][j].getValue(0),
                        m_TriGrid[i][j].getValue(1)
                        );
            }
    ...
    /*output all the vertice in some level*/
    void GEarthRadiationModel::outputVertice_js( std::vector<GVertex>& vertice, FILE* pf)
    {
        ...
        for(int i = 0 ; i< vertice.size() ; i++ )
        {
            if( i == vertice.size() - 1 )
            {
               fprintf(pf, "[ %16.4f , %16.4f , %16.4f , %12.6f , %12.6f]\n",vertice[i].getX(),vertice[i].getY(),vertice[i].getZ(),vertice[i].getLon(),vertice[i].getLat() );
            }
            else
            {
                fprintf(pf, "[ %16.4f , %16.4f , %16.4f , %12.6f , %12.6f],\n",vertice[i].getX(),vertice[i].getY(),vertice[i].getZ(),vertice[i].getLon(),vertice[i].getLat());
    ...
    /*output all the vertice in some level*/
    void GEarthRadiationModel::outputTri_js( std::vector<GTriangle<GVertex> >& tri, FILE* pf)
    {
        ...
        for( int i = 0 ;i < tri.size(); i++ )
        {
            ...
            {
                fprintf(pf, "[ %6d , %6d , %6d ]\n",
                        tri[i].getPoint(0).getName().asINT()-1,
                        tri[i].getPoint(1).getName().asINT()-1,
                        tri[i].getPoint(2).getName().asINT()-1);
            }
            else
            {
                fprintf(pf, "[ %6d , %6d , %6d ],\n",
...
//        int testc =0;
    ...
    //attach the original grid onto the triangle grids
    void GEarthRadiationModel::attachFlux(int imonth,int type)
    {
        
        double Ra = m_Ra;//6408137;     // a = 6378137 for WGS84
        double Rb = m_Ra;//6386651.7;
        ...
        gfc::GEarthRadiationModel::FLUXDATA& fluxdata = m_monToaData[type][imonth];
        GString path = "data/";
        char strtmp1[3]={0};
        sprintf(strtmp1, "%02d",imonth+1);
        GString filename;
        if( type == 0 )  //longwave
        {
            filename = path + "flux/" + GString(strtmp1) +"/longwave" + GString(strtmp1) +".grid";
            
        }
        else if(type == 1) //shortwave
        {
            filename = path + "flux/" + GString(strtmp1) +"/shortwave" + GString(strtmp1) +".grid";
        }
        
        FILE*  gridFile = fopen(filename.c_str(),"w+");
        
        
        //test the total energy
        double total_grid = 0.0, total_tri = 0.0;
        for(int t1 = 0 ; t1 < 360; t1++ )
        {
            for(int t2= 0; t2< 180; t2++ )
            {
                //double lat = -89.5+t1;
                //double lon = 0.5+ t2;
                ...
                //double perimeter0 =0.0, area =0.0;
                //poly.Compute(false, true, perimeter0, area);
                //poly.Clear();
                
                //total_grid += fluxdata.data[t2][t1];
                fprintf(gridFile, "%20.10f ", fluxdata.data[t2][t1]);
            }
            fprintf(gridFile, "\n");
        }
        ...
        for( int i = 0; i < m_level+1 ; i++ )
        {
            ...
            GString tmp1 = "level";
            GString tmp2;
            tmp1 = tmp1 + GString(strtmp);
            tmp2 = path + tmp1;
            tmp2 = tmp2 + "polygon.py";
            FILE*  polygonFile = fopen(tmp2.c_str(),"w+");
            ...
            //for( int j = 50 ; j < 51; j++ )
            {
                ...
                double flux =  countingStars(poly ,p, fluxdata,polygonFile);
                
                m_TriGrid[i][j].setValue(flux,type);
...
double GEarthRadiationModel::getAngle(double* pA, double* pC)
    {
    ...
    /*
     ...
     ref: http://mathforum.org/library/drmath/view/54680.html
     */
    double GEarthRadiationModel::GCDistance(double lat1, double lon1, double lat2, double lon2,double radius)
    {
     ...
     */
void GEarthRadiationModel::gridIndex( double min[2], double max[2],double interval[2], double xy[2], int* pos  )
    {
        int tmp1 = ceil(xy[1]/interval[1])-1;  // 大于或等于的整数
...
std::vector<std::vector<double> > GEarthRadiationModel::polygonPointsProcess(std::vector<std::vector<double> >& allpoints)
    {
     ...
     */
void GEarthRadiationModel::ClockwiseSortPoints(std::vector<std::vector<double> > &Points)
    {
        if( Points.size() <= 0 )
        {
            return;
    ...
    /**/
void GEarthRadiationModel::determinGrid(double lat, double lon, double* interval, double* min, double* max,double* starter)
    {
        int tag[2] ={0};
...
bool GEarthRadiationModel::IsPointInTri_sph( GVertex p[3] , double lat, double lon)
     ...
     */
bool GEarthRadiationModel::visiabilityTest( GTriangle<GVertex>& tri ,double* pos)
    {
        double PI = GCONST("PI");
        bool test = false;
        double m[9]={0.0};
        ...
        {
            int minIndex1 = 0;
            double minDistance1 = 9999000; //至少40000km 最下的距离意味着最大的高度角
            double minDistance2 = 9999000; //至少40000km 最下的距离意味着最大的高度角
            ...
            for( int i = 0 ; i< 3; i++ )
            {
                double dis = GCDistance( tpoints[i].getLat() , tpoints[i].getLon(), subsatpoint[0], subsatpoint[1], m_Ra/1000.0);
                if( dis < minDistance1 )
                {
                    minIndex1 = i;
                    minDistance1 = dis;
            ...
            for( int j = 0 ; j< 3; j++ )
            {
                ...
                double dis  = theta*m_Ra/1000.0;
                
                //check whether this point is inside the triangle
                neareatPointXYZ[0] = d[0]/len3*m_Ra;
                neareatPointXYZ[1] = d[1]/len3*m_Ra;
                neareatPointXYZ[2] = d[2]/len3*m_Ra;
                ...
                {
                    minDistance2 = dis;
                    neareatPointXYZ[0] = d[0]/len2*m_Ra;
                    neareatPointXYZ[1] = d[1]/len2*m_Ra;
                    neareatPointXYZ[2] = d[2]/len2*m_Ra;
                }
            }
     ...
     *
     */
void  GEarthRadiationModel::visibleArea( double* pos, std::vector< std::vector<GTriangle<GVertex> > >& alltri,std::vector<GTriangle<GVertex> >& myres )
    {
        ...
        //output myres
        FILE* visibleArea = fopen("visible.txt", "w+");
       // fprintf(visibleArea, "%s\n","Points=[");
        for( int i = 0 ; i< myres.size(); i++ )
        {
            myres[i].write2File(visibleArea);
        }
...
double GEarthRadiationModel::countingStars(PolygonAreaExact poly, gfc::GVertex *p, gfc::GEarthRadiationModel::FLUXDATA fluxdata, FILE *polygonFile)
    {
        ...
        double area0 = 0.0 ; // the area of the triangle
        poly.AddPoint(p[0].getLat(), p[0].getLon());
        poly.AddPoint(p[1].getLat(), p[1].getLon());
        poly.AddPoint(p[2].getLat(), p[2].getLon());
        double perimeter0 =0.0;
        poly.Compute(false, true, perimeter0, area0);
        poly.Clear();
        ...
        while(1)
        {
            ...
            if( tmpvalue.size() == 0 )
            {
                ...
                centerPoint[0] = centerPoint[0]/3.0;
                centerPoint[1] = centerPoint[1]/3.0;
            ...
            for( ; latlooper < upStarter;  latlooper = latlooper + interval[0] )
            {
                ...
                if( myarea2 < 0 )
                {
                    printf("warning: area2 is negative!!\n");
                }
                poly.Clear();
                ...
                double ratio = myarea1/area0;  // should use the area of the total triangle
    ...
    {
        int type = -1;
        if( m_varableName == "toa_lw_all_mon")
        {
            type = 0;
        }
        else if(m_varableName == "toa_sw_all_mon")
        {
            type = 1;
        }
        ...
        for( int rec = 0 ; rec< dim_time_len; rec++ )
        {
            ...
            for( int i = 0 ; i< dim_lat_len; i++ )
            {
                ...
                {
                    m_monToaData[type][myct.m_month-1].data[i][j] += toaData[i][j];
                }
            }
        }
        ...
        for( int k =0 ; k< m_monToaData[type].size() ; k++ )
        {
            for( int i = 0 ; i<dim_lon_len; i++  ) //360
            {
                for( int j = 0 ; j< dim_lat_len; j++ ) //180
                {
                    m_monToaData[type][k].data[j][i] = m_monToaData[type][k].data[j][i] / dataTag[k];
                }
            }
        }
        
        ///double c = m_monToaData[0][0].data[0][3];
    ...
    /*
     *
     * mydata is used to store the 12 month data
     *std::vector< GTriangle<POINT> >
     */
 void GEarthRadiationModel::readingTriGridFile( gfc::GString filename, std::vector< GTriangle<GVertex> >&  mydata)
    {
        std::vector< GTriangle<GVertex> > test;
        ...
        while( !fs.eof() )
        {
            ...
            if( split_str.size() == 13 )  // the last level
            {
                GVertex p1("",split_str[2].asDOUBLE(),split_str[3].asDOUBLE(),split_str[4].asDOUBLE());
                GVertex p2("",split_str[5].asDOUBLE(),split_str[6].asDOUBLE(),split_str[7].asDOUBLE());
                GVertex p3("",split_str[8].asDOUBLE(),split_str[9].asDOUBLE(),split_str[10].asDOUBLE());
                
                GTriangle<GVertex>  mytri("",p1,p2,p3);
                mytri.setTag(split_str[0]);
                mytri.setFtag(split_str[1]);
                GString tstr[4]={""};
                mytri.setCtag(tstr);
                mytri.setValue(split_str[11].asDOUBLE(),0);
                mytri.setValue(split_str[12].asDOUBLE(),1);
                mydata.push_back(mytri);
                //test.push_back(mytri);
                
                int testc = 0;
                
            }
            else if( split_str.size() == 17 ) // the normal level
            {
                
                GVertex p1("",split_str[6].asDOUBLE(),split_str[7].asDOUBLE(),split_str[8].asDOUBLE());
                GVertex p2("",split_str[9].asDOUBLE(),split_str[10].asDOUBLE(),split_str[11].asDOUBLE());
                GVertex p3("",split_str[12].asDOUBLE(),split_str[13].asDOUBLE(),split_str[14].asDOUBLE());
                
                GTriangle<GVertex>  mytri("",p1,p2,p3);
                mytri.setTag(split_str[0]);
                mytri.setFtag(split_str[1]);
                GString tstr[4]={split_str[2],split_str[3],split_str[4],split_str[5]};
                mytri.setCtag(tstr);
                mytri.setValue(split_str[15].asDOUBLE(),0);
                mytri.setValue(split_str[16].asDOUBLE(),1);
...
/*
 * the main entry to create the flux data files
 *
 */
void GEarthRadiationModel::createFluxModel()
    {
        //GEarthRadiationModel radiationModel;
        createGrid();
        
        setVariableName("toa_lw_all_mon");
        // load the netcdf datafile
        loadData();
        
        setVariableName("toa_sw_all_mon");
        // load the netcdf datafile
        loadData();
        
        //SphericalHarmonicsTest();
        
        GString dir = "data/flux/";
        for( int i = 0 ; i< 12; i++ )
        {
            printf("longwave month:%02d\n",i);
            attachFlux(i,0);  // from Jan to Dec
            printf("shortwave month:%02d\n",i);
            attachFlux(i,1);  // from Jan to Dec
            printf("output the flux\n");
            dumpTriGridFlux(i,dir);
        }
    ...
    void GEarthRadiationModel::SphericalHarmonicsTest()
    {
        double D2R = GCONST("D2R");
        
        gfc::GEarthRadiationModel::FLUXDATA& fluxdata = m_monToaData[0][0];
        
        int countD = 180*360;
        double l[180*360]={0.0};
        int count = 0;
        int N = 20, M = 20;
        ...
        double* tempX = new double[countX];
        
        GMatrix bl(countX,1);
        GMatrix bb(countX,countX);
        ...
        for( int i = 0 ; i< 180; i++)
        {
            for( int j = 0 ; j< 360; j++ )
            {
                ...
                l[count] = fluxdata.data[i][j];
                
                sp.Sph_Harmonics(lat*D2R, lon*D2R, tempX);
                
                GMatrix t(tempX,1,countX);
                //printf("\nT:\n");
                //t.dump();
                GMatrix tt = ~t*t;
                bb += tt;
                ...
                {
                    bl[k] += tempX[k]*l[count];
                    b[count*countX+k] = tempX[k];
                }
        ...
        GMatrix B(b,countD,countX);
        
        GMatrix L(l,countD,1);
        ...
        delta = sqrt(delta / (countD - countX));
...
//    GEarthRadiationModel radiationModel;
//    
//    radiationModel.onlineTest();
//    
//    radiationModel.createGrid();
//    
//    radiationModel.setVariableName("toa_lw_all_mon");
//    radiationModel.loadData();
//    
//    // should give out the position of spacecraft and time
//    double testpos[3] = { -7247484.740, 14497341.270, -22684596.470};
//    
//    GString dir = "../data/longwave/";
//    for( int i = 0 ; i< 12; i++ )
//    {
//        printf("longwave month:%02d\n",i);
//        radiationModel.attachFlux(i);  // from Jan to Dec
//        radiationModel.dumpTriGridFlux(i,dir);
//        radiationModel.dumpMonToaData(dir);
//    }
//    
//    radiationModel.setVariableName("toa_sw_all_mon");
//    radiationModel.loadData();
//    dir = "../data/shortwave/";
//    for( int i = 0 ; i< 12; i++ )
//    {
...
void getFluxDataTest(gfc::GString netcdfname)
{
    ...
    gfc::GEarthRadiationModel radiationModel;
    radiationModel.loadData();
    radiationModel.createGrid();
    double testpos[3] = { -7247484.740, 14497341.270, -22684596.470};
    //radiationModel.attachFlux(0);
